...
import { userThunks } from "@/entites/user/store/userSlice";
...
export const AuthPage = ({ isLogin }: { isLogin: boolean }) => {
  ...
  const handleRegisterSubmit = async (fields: Required<AuthFormFields>) => {
    try {
      await dispatch(userThunks.registerUser(fields)).unwrap();
